...
def parse_arguments():
    ...
    with open(parser.parse_args().input_file, "r") as f:
        args = json.load(f)
    return args


def main(mat_file: str, k: int):
    import copy
    data = scipy.io.loadmat(mat_file)['P']
    dataset = ds.Dataset.yun_14(k)
    parameters = ds.Parameters(
        batch_capacity=30, max_workers=10, enable_random_rep=False, cap=2000,
        eta=0.0001, delta=0.3,
...
if __name__ == "__main__":
    ...
    base = '../mat'
    for k in [3, 20, 30, 90]:
        mat_file = os.path.join(base, f'pair_prob_mat_K={k}.mat')
        for i in range(10):
            tracker = main(mat_file, k)
            tracker.as_pd().to_csv(os.path.join(
                base, f'simulation_K={k}_i={i}.csv'))
